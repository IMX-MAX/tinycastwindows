...
of Liquid Glass and **Mistral** for every AI feature.

Download **[TinycastSetup.exe](windows/dist/TinycastSetup.exe)** (Windows 10/11, 64-bit) and run it.
`Alt+Space` summons the palette. Paste a Mistral API key under Settings → AI.

Rebuild with `./windows/Scripts/build-installer.sh` (needs the .NET 8 SDK and NSIS).
